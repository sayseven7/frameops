...
func (bucket Bucket) EnsureBucket(ctx context.Context) error {
	response, err := bucket.send(ctx, http.MethodPut, "", nil, 0, emptyPayloadHash, "")
	if err != nil {
...
func (bucket Bucket) Put(ctx context.Context, key string, body io.Reader, size int64, payloadSHA256, contentType string) error {
	response, err := bucket.send(ctx, http.MethodPut, key, body, size, payloadSHA256, contentType)
	if err != nil {
...
func (bucket Bucket) Get(ctx context.Context, key string) (io.ReadCloser, error) {
	response, err := bucket.send(ctx, http.MethodGet, key, nil, 0, emptyPayloadHash, "")
	if err != nil {
...
func (bucket Bucket) send(ctx context.Context, method, key string, body io.Reader, size int64, payloadSHA256, contentType string) (*http.Response, error) {
	if key != "" && !validKey(key) {
