...
import { approveReportRevision, captureEvidence, collectionItems, createEngagement as createEngagementRequest, createFinding, createMethodology, deriveReportPDF, publishMethodology, readEvidence, readEngagementChecklist, readIngestions, readReportRevisions, recordRetest, triageFinding, type Client, type Engagement, type EngagementChecklist, type Evidence, type Finding, type FindingInput, type Ingestion, type Methodology, type MethodologyInput, type ReportPDF, type ReportRevision, type Retest, type RetestInput, requestJSON, uploadReportRevision } from "./api";
    ...
    <main className={signedIn ? "app-shell" : "login-shell"}>
      {!signedIn ? <section className="login-panel" aria-labelledby="sign-in-title">
        ...
        <section className="app-content" ref={contentRef} tabIndex={-1} aria-label={navigation.find((item) => item.id === section)?.label}>
          ...
          {section === "imports" && <section className="workspace" aria-labelledby="imports-title"><header className="surface-header"><div><h1 id="imports-title">{text.imports}</h1><p>{text.importsDescription}</p></div></header>{!engagementID ? <div className="empty-state standalone"><strong>{text.selectEngagement}</strong><p>{text.importsNeedProject}</p></div> : <><section className="data-surface import-surface"><div className="table-head import-grid"><span>{text.artifact}</span><span>{text.importSource}</span><span>{text.importReceivedAt}</span><span>{text.importResult}</span><span>{text.importState}</span></div>{ingestions.map((ingestion) => <div className="data-row import-grid" key={ingestion.id}><div><strong>{ingestion.filename}</strong><small>{formatBytes(ingestion.byteSize)} · <code title={ingestion.sha256}>{ingestion.sha256}</code></small></div><span>{ingestion.tool} · {ingestion.formatVersion}</span><time dateTime={ingestion.receivedAt}>{new Intl.DateTimeFormat(locale, { dateStyle: "short", timeStyle: "short" }).format(new Date(ingestion.receivedAt))}</time><span>{ingestion.summary.created} {text.importCreated} · {ingestion.summary.reused} {text.importReused} · {ingestion.summary.ignored} {text.importIgnored} · {ingestion.summary.rejected} {text.importRejected}</span><span className="status status-active">{text.importRecorded}</span></div>)}{!ingestions.length && <div className="empty-state"><strong>{text.noImports}</strong><p>{text.importsCommand}</p></div>}</section><p className="cli-command"><code>fops ingest nmap ./scan.xml --engagement {engagementID}</code></p></>}</section>}
