      ...
      {!signedIn ? <section className="login-panel" aria-labelledby="sign-in-title">
        ...
        <section className="app-content" ref={contentRef} tabIndex={-1} aria-label={navigation.find((item) => item.id === section)?.label}>
          ...
          {section === "overview" && <section className="workspace" aria-labelledby="overview-title">
            <header className="surface-header"><div><h1 id="overview-title">{selectedEngagement?.name ?? text.workspace}</h1><p>{selectedEngagement ? `${selectedClient?.name} / ${text.projectSummary}` : text.workbenchDescription}</p></div>{engagementID && <button type="button" className="secondary-action" onClick={() => setSection("findings")}>{text.openFindings}</button>}</header>
            ...
            <div className="overview-grid">
              <section className="surface-map" aria-labelledby="surface-title"><header><div><h2 id="surface-title">{text.attackSurface}</h2><p>{text.attackSurfaceDescription}</p></div><span className={engagementID ? "scope-state active" : "scope-state"}>{engagementID ? text.scopeLoaded : text.noActiveProject}</span></header>
                {engagementID ? <div className="map-body"><div className="scope-core"><span>{text.authorizedScope}</span><strong>{selectedEngagement?.name}</strong><small>{selectedClient?.name}</small></div><div className="finding-nodes">{findings.length ? findings.map((finding) => <button type="button" key={finding.id} className={finding.id === findingID ? "finding-node selected" : "finding-node"} onClick={() => { void selectFinding(finding.id); setSection("findings"); }}><span>{text.score} {finding.cvssScore}</span><strong>{finding.title}</strong><small>{finding.validationState} · {finding.remediationState ?? "new"}</small></button>) : <div className="map-empty"><strong>{text.noFindings}</strong><button type="button" className="text-action" onClick={() => setSection("findings")}>{text.recordFirstFinding}</button></div>}</div></div> : <div className="empty-state"><strong>{text.selectOperationalContext}</strong><p>{text.noEngagementSelected}</p></div>}
              ...
              <aside className="action-queue" aria-labelledby="queue-title"><header><h2 id="queue-title">{text.actionQueue}</h2><span>{queueTotal}</span></header>{engagementID ? <ol><li><button type="button" onClick={() => setSection("findings")}><span><strong>{text.triageQueue}</strong><small>{text.triageQueueHelp}</small></span><b>{queue.triage}</b></button></li><li><button type="button" onClick={() => setSection("evidence")}><span><strong>{text.retestQueue}</strong><small>{text.retestQueueHelp}</small></span><b>{queue.retest}</b></button></li><li><button type="button" onClick={() => setSection("reports")}><span><strong>{text.approvalQueue}</strong><small>{text.approvalQueueHelp}</small></span><b>{queue.approval}</b></button></li><li><button type="button" onClick={() => setSection("reports")}><span><strong>{text.deliveryQueue}</strong><small>{text.deliveryQueueHelp}</small></span><b>{queue.delivery}</b></button></li></ol> : <div className="empty-state compact"><p>{text.queueNeedsProject}</p></div>}</aside>
            </div>
